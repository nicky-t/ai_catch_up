...
## 現在地（最新の要約）
<!-- 週次で更新される 3〜5 行 -->
- 「導入した」から「組織で使い回す」段階への移行事例が今週いくつも重なった：トラコムは属人化していたプロンプトを社内共有資産に集約し資料作成を月32時間→4時間に短縮、あいち銀行はLayerXの請求書受領サービスを決済基盤に組み込み、キユーピーはAIエージェント「Devin」で製造シミュレーション工数を78.8%削減。9/22の双日調査（Copilot導入企業の87.5%が「情報分散」を課題視）が示した壁を、具体的な業務組み込みで越え始めている組織が目立つ
- 一方で「導入率」と「使いこなし」の溝は数字でも裏付けられ続けている：クラウドワークス調査ではAI導入企業が約6割に達しつつも8割超が「チャット止まり」で業務自動化はわずか19.1%。研修面でもNEXER/LISKILLING調査でAI・DX研修の受講経験者は1割未満（希望者は4割超）と、学びたい人と機会のギャップも明らかに
- パーソルが「データ基盤問題」（品質・権限管理・部署間定義の相違）への支援3サービスを開始するなど、「PoC止まり」の根本原因への対応も具体化してきている
- 大企業・自治体の先行事例（三菱UFJ銀行のルール構造化、首都高の全社活用、千代田区の全庁展開・豊川信用金庫のFAQ化）は今週新規の動きなし。国産オープンモデル（NIIのLLM-jp-4 33B）も同様に動きなし
